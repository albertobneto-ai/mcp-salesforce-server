...
export class SalesforceClient {
  ...
  async describeGlobal() { return await this.conn.describeGlobal(); }
  async describeObject(objectName) { return await this.conn.sobject(objectName).describe(); }
  async query(soql) { return await this.conn.query(soql); }
  ...
  async readMetadata(type, fullNames) { return await this.conn.metadata.read(type, fullNames); }
  ...
  async deployManifest(manifest, checkOnly = false) {
          ...
          if (success) {
            ...
            if (obj.fields?.length) {
              for (const field of obj.fields) {
                ...
                try {
                  const fieldMeta = {
                    fullName: obj.fullName + "." + field.fullName, label: field.label, type: field.type,
                    ...(field.length && { length: field.length }),
                    ...(field.required && { required: field.required }),
                    ...(field.externalId && { externalId: field.externalId }),
                    ...(field.formula && { formula: field.formula }),
                    ...(field.formulaTreatBlanksAs && { formulaTreatBlanksAs: field.formulaTreatBlanksAs }),
                    ...(field.referenceTo && { referenceTo: field.referenceTo }),
                    ...(field.relationshipName && { relationshipName: field.relationshipName }),
                    ...(field.precision && { precision: field.precision }),
                    ...(field.scale && { scale: field.scale }),
                    ...(field.picklist && { valueSet: { valueSetDefinition: { value: field.picklist.map(v => ({ fullName: v, label: v, default: false })) } } }),
                  };
                  const fResult = await this.conn.metadata.upsert("CustomField", fieldMeta);
    ...
    if (manifest.metadata?.customFields?.length) {
      for (const field of manifest.metadata.customFields) {
        ...
        try {
          const fieldMeta = {
            fullName: field.fullName, label: field.label, type: field.type,
            ...(field.length && { length: field.length }),
            ...(field.required && { required: field.required }),
            ...(field.formula && { formula: field.formula }),
            ...(field.formulaTreatBlanksAs && { formulaTreatBlanksAs: field.formulaTreatBlanksAs }),
